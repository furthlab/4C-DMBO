data<-read.table('kinetics/4C-DMBO/BD2-30mM.csv', header=T, skip=15, sep=',')
...
k_results <- extract_k_values("kinetics/4C-DMBO/")
print(k_results)

# Molarity of azide across conditions
molarity <- c(0.03, 0.039, 0.051, 0.066, 0.066, 0.07)  # in M
...
plot_all_uvvis <- function(folder_path) {
  ...
  for (f in files) {
    ...
    plot(data$Time.s., data$Absorbance, 
         type = 'l', ylab = 'Absorbance (309 nm)', xlab = 'Time (sec.)', 
...
plot_all_uvvis('kinetics/4C-DMBO/')
